...
public class DevToolsService : MonoBehaviour {
    ...
    void FixedUpdate(){
        for(int i = 0; i < DevTools.ListGameObjects.Count; i++){
            if(!DevTools.ListGameObjects.ElementAt(i).Value){
                DevTools.ListWindows.Remove(DevTools.ListGameObjects.ElementAt(i).Key);
                DevTools.ListGameObjects.Remove(DevTools.ListGameObjects.ElementAt(i).Key);
            }
        }
    }
    ...
    void Update(){
        ...
        if(Input.GetKeyDown(KeyCode.F1)){
            DevTools.isOpenDevTools = !DevTools.isOpenDevTools;
            DevTools.isOverlays = DevTools.isOpenDevTools;
            DevTools.SelectedObject = null;
        }

        if(Input.GetKeyDown(KeyCode.F2))
            DevTools.isOpenDeveloperTools = !DevTools.isOpenDeveloperTools;
    ...
    void OnGUI(){
        if(Debug.isDebugBuild){
            ...
            foreach(var item in DevTools.ListGameObjects.Values.Distinct())
                if(item)
                    DevTools.DrawString(item.name, item.transform.position, Color.white, Texture2D.grayTexture);
            ...
            if(DevTools.isOpenDevTools){
                ...
                if(DevTools.SelectedObject)
                    GUILayout.Window(3, new Rect(SizePerformance.x + PaddingScreen.x * 2,  PaddingScreen.y * 2 + SizePerformance.y, SizeComponents.x, SizeComponents.y), DevToolsWindow, DevTools.SelectedObject.name == gameObject.name ? currentList.ToString() : DevTools.SelectedObject.name);
            }else{
                GUILayout.Label("  Press F1 to open/close DevTools." + (!DevTools.CurrentWindow.Equals(new KeyValuePair<string, GUI.WindowFunction>()) ? "\n  Press F2 to open/close current Developer Tools." : "") + "\n  Press F3 to show/hide Overlays.");
            }
            if(DevTools.isOpenDeveloperTools)
            if(!DevTools.CurrentWindow.Equals(new KeyValuePair<string, GUI.WindowFunction>()))
                GUILayout.Window(4, new Rect(Screen.width - SizeInspector.x - PaddingScreen.x, PaddingScreen.y, SizeInspector.x, SizeInspector.y), DevTools.CurrentWindow.Value, DevTools.CurrentWindow.Key);
        }
    }
        ...
        switch(id){
            ...
            case 1: // Manager
                ...
                if(GUILayout.Button("Scenes")){
                    ...
                    DevTools.SelectedObject = gameObject;
                }
            ...
            case 2: //DevTools
                ...
                foreach(var item in DevTools.ListGameObjects.Values.Distinct().Where(item => item))
                    if(GUILayout.Button(item.name)){
                        currentList = typeList.Components;
                        DevTools.SelectedObject = item;
                    }
                 GUILayout.EndScrollView();
            break;
            case 3: //Component
    ...
    void Draw(){
        ...
        switch(currentList){
            case typeList.Components:
               quantity = DevTools.ListGameObjects.Where(item => item.Value == DevTools.SelectedObject).Count();
            break;
        ...
        switch(currentList){
            case typeList.Components:
                var Components = DevTools.ListGameObjects.Where(item => item.Value == DevTools.SelectedObject);
                foreach(var item in Components)
                    if(GUILayout.Button(item.Key)){
                        DevTools.isOpenDeveloperTools = true;
                        DevTools.CurrentWindow = DevTools.ListWindows.First(item2 => item2.Key == item.Key);
                    }
            break;
            case typeList.Scenes:
                ...
                for(int i = 0; i < SceneManager.sceneCountInBuildSettings; i++)
                    if(GUILayout.Button(regex.Replace(SceneUtility.GetScenePathByBuildIndex(i), "$2"))){
                        DevTools.CurrentWindow = new KeyValuePair<string, GUI.WindowFunction>();
